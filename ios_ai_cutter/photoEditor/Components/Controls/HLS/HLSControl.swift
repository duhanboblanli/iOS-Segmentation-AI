...
struct HLSControl: View {
    ...
    var body: some View {
        ...
        return VStack(alignment: .leading){
            ...
            Spacer()
            FilterSlider(value: intensityHeu, range: (-0.2, 0.2), label: "Hue".localized(LocalizationService.shared.language)
, defaultValue: 0)
            FilterSlider(value: intensitySaturation, range: (0, 2), label: "Saturation".localized(LocalizationService.shared.language)
, defaultValue: 1)
            FilterSlider(value: intensityLuminance, range: (0.5, 1.5), label: "Luminance".localized(LocalizationService.shared.language)
, defaultValue: 1)
            Spacer()
        }
