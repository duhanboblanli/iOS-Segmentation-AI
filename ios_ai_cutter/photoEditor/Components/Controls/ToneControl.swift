...
struct ToneControl: View {
    ...
    var body: some View {
        ...
        return VStack(spacing: 24){
            FilterSlider(value: highlight, range: (FilterHighlights.range.min, FilterHighlights.range.max), label: "Highlights".localized(LocalizationService.shared.language)
, defaultValue: 0, rangeDisplay: (0, 100), spacing: 8)
            
            FilterSlider(value: shadow, range: (FilterShadows.range.min, FilterShadows.range.max),label: "Shadows".localized(LocalizationService.shared.language)
, defaultValue: 0, spacing: 8)
        }
