...
struct WhiteBalanceControl: View {
    ...
    var body: some View {
        ...
        return ZStack {
            VStack(spacing: 24){
                FilterSlider(value: temperature, range: (FilterWhiteBalance.range.min, FilterWhiteBalance.range.max), label: "Temperature".localized(LocalizationService.shared.language)
, defaultValue: 0, spacing: 8)
                
                FilterSlider(value: tint, range: (FilterWhiteBalance.range.min, FilterWhiteBalance.range.max),label: "Tint".localized(LocalizationService.shared.language)
, defaultValue: 0, spacing: 8)
            }
        }
